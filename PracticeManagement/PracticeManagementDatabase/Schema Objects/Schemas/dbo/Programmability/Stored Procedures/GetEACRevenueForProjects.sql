CREATE PROCEDURE [dbo].[GetEACRevenueForProjects]
(
...
FROM #ActualTimeEntries AS AE --ActualEntriesByPerson
	...
	SELECT	f.ProjectId,
		   ...
		   SUM(CASE WHEN f.IsHourlyAmount = 0 and f.Date > @LastMonthEnd THEN f.PersonMilestoneDailyAmount 
				    WHEN f.IsHourlyAmount = 1 and f.Date > @Today THEN f.PersonMilestoneDailyAmount
					ELSE 0 END ) AS RemRevenuePerDay
	...
	SELECT  CT.ProjectId, 
			CONVERT(INT,SUM(ISNULL(CT.HourlyActualRevenuePerDay, 0) + ISNULL(CT.FixedActualRevenuePerDay, 0)+ ISNULL(CT.RemRevenuePerDay,0))) AS EACRevenue
