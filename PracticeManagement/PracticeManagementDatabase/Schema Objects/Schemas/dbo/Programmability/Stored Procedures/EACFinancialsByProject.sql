CREATE PROCEDURE [dbo].[EACFinancialsByProject]
(
	@ProjectId   INT
)
AS
...
FROM #ActualTimeEntries AS AE --ActualEntriesByPerson
	...
	SELECT	@ProjectIdLocal AS ProjectId,
			f.personid,
			f.Date,
		   SUM(case WHEN f.Date > @LastMonthEnd THEN f.PersonMilestoneDailyAmount ELSE 0 END) AS Revenue,
	       SUM(case WHEN f.Date > @LastMonthEnd THEN (f.PersonMilestoneDailyAmount - f.PersonDiscountDailyAmount) ELSE 0 END) AS RevenueNet,
		   SUM(case WHEN f.Date > @LastMonthEnd THEN( f.PersonMilestoneDailyAmount - f.PersonDiscountDailyAmount -
						(CASE WHEN f.SLHR  >= f.PayRate + f.MLFOverheadRate 
							  THEN f.SLHR ELSE f.PayRate + f.MLFOverheadRate END)*ISNULL(f.PersonHoursPerDay, 0)) ELSE 0 END) GrossMargin,
	...
	INTO #ActualAndRemainingProjectedValues
	FROM #FinancialsRetro AS f
	GROUP BY f.Date, f.personid,
			f.Date

	CREATE CLUSTERED INDEX CIX_ActualAndProjectedValuesDaily ON #ActualAndRemainingProjectedValues(ProjectId)
	...
	FROM #ActualAndRemainingProjectedValues CT


 SELECT 5 AS FinanceType,
			APV.ProjectId,
			CONVERT(DECIMAL(18,2), ISNULL(APV.ActualRevenue,0)+ISNULL(APV.Revenue,0)) Revenue,
			CONVERT(DECIMAL(18,2),(ISNULL(APV.ActualRevenue,0) +ISNULL(AE.ActualReimbursement,0)-(ISNULL(APV.ActualRevenue,0) * ISNULL(APV.Discount,0)/100))+(ISNULL(APV.RevenueNet,0)+ISNULL(AE.EstimatedReimbursement,0))) AS 'RevenueNet',
			--CONVERT(DECIMAL(18,2),(ISNULL(APV.ActualRevenue,0) -ISNULL(APV.ActualMargin,0))+ISNULL(APV.Cogs,0)) AS 'Cogs',
			CONVERT(DECIMAL(18,2), (ISNULL(APV.ActualMargin,0) - (ISNULL(APV.ActualRevenue,0) * ISNULL(APV.Discount,0)/100) + ((ISNULL(AE.ActualReimbursement,0)-ISNULL(AE.ActualExpense,0))))+ISNULL(APV.GrossMargin,0)+((ISNULL(AE.EstimatedReimbursement,0)-ISNULL(AE.EstimatedAmount,0)))) 'GrossMargin'
			--ISNULL(AE.ActualExpense,0)+ ISNULL(AE.EstimatedAmount,0) Expense,
			--ISNULL(AE.ActualReimbursement,0)+ ISNULL(AE.EstimatedReimbursement,0) ReimbursedExpense
	...
	DROP TABLE #ActualAndRemainingProjectedValues
