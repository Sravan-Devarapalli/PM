CREATE PROCEDURE [dbo].[GetEACFinancialsByProjectIds]
(
...
BEGIN
	...
	JOIN (
			...
			FROM Project Pro
				INNER JOIN @ProjectIDs pr on pr.ResultId=pro.ProjectId
	...
	SELECT  m.ProjectId,
			...
			FROM dbo.Project P
			INNER JOIN @ProjectIDs pr on pr.ResultId=p.ProjectId
...
FROM #ActualTimeEntries AS AE --ActualEntriesByPerson
	...
	INNER JOIN @ProjectIDs p on p.ResultId=f.ProjectId


	SELECT PDE.ProjectId AS ProjectId,
	...
	INNER JOIN @ProjectIDs p on p.ResultId=PDE.ProjectId
	...
	GROUP BY f.ProjectId,f.Date, f.personid,
			f.Date
