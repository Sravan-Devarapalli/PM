...
AS
	...
	*/
	...
	DECLARE @FirstRecord INT,
			 @LastRecord INT,
			 @PersonLastFirstName NVARCHAR(85)
	  ...
	  FROM (
	        SELECT TOP (@LastRecord)
	         ...
	         WHERE CONVERT(DATE,a.LogDate) BETWEEN CONVERT(DATE,@StartDate) AND CONVERT(DATE,@EndDate)
				 ...
				 --OR(
				 --((@EventSource = 'Vendor'  OR @EventSource = 'All') 
					--		AND (a.LogData.exist('/Vendor') = 1 
					--			OR a.LogData.exist('/VendorAttachment') = 1
					--			 )
					--	)
					--	AND 
					--	(@VendorId IS NULL 
					--		OR (a.LogData.value('(/Vendor/NEW_VALUES/@VendorId)[1]', 'int') = @VendorId 
					--			AND ( ((CASE WHEN (a.LogData.exist('(/Vendor)') = 1) AND CONVERT(XML, a.Data).value('(/Project/NEW_VALUES/@Name)[1]', 'NVARCHAR(100)') <> CONVERT(XML, a.Data).value('(/Vendor/NEW_VALUES/OLD_VALUES/@Name)[1]', 'NVARCHAR(100)')
					--				   THEN 1 
					--				   ELSE 0 END) +
					--				   (CASE WHEN (CONVERT(XML,a.Data).exist('(/Vendor)') = 1)  AND CONVERT(XML, a.Data).value('(/Vendor/NEW_VALUES/@ContactName)[1]', 'NVARCHAR(100)') <> CONVERT(XML, a.Data).value('(/Vendor/NEW_VALUES/OLD_VALUES/@ContactName)[1]', 'NVARCHAR(100)')
					--					THEN 1 
					--					ELSE 0 END) +
					--					(CASE WHEN (CONVERT(XML,a.Data).exist('(/Vendor)') = 1)  AND CONVERT(XML, a.Data).value('(/Vendor/NEW_VALUES/@Email)[1]', 'NVARCHAR(100)') <> CONVERT(XML, a.Data).value('(/Vendor/NEW_VALUES/OLD_VALUES/@Email)[1]', 'NVARCHAR(100)')
					--					THEN 1 
					--					ELSE 0 END) +
					--					(CASE WHEN (CONVERT(XML,a.Data).exist('(/Vendor)') = 1)  AND CONVERT(XML, a.Data).value('(/Vendor/NEW_VALUES/@TelephoneNumber)[1]', 'NVARCHAR(100)') <> CONVERT(XML, a.Data).value('(/Vendor/NEW_VALUES/OLD_VALUES/@TelephoneNumber)[1]', 'NVARCHAR(100)')
					--					THEN 1 
					--					ELSE 0 END) +
					--					(CASE WHEN (CONVERT(XML,a.Data).exist('(/Vendor)') = 1)  AND CONVERT(XML, a.Data).value('(/Vendor/NEW_VALUES/@VendorType)[1]', 'NVARCHAR(100)') <> CONVERT(XML, a.Data).value('(/Vendor/NEW_VALUES/OLD_VALUES/@VendorType)[1]', 'NVARCHAR(100)')
					--					THEN 1 
					--					ELSE 0 END) +
					--					(CASE WHEN (CONVERT(XML,a.Data).exist('(/Vendor)') = 1) AND CONVERT(XML, a.Data).value('(/Vendor/NEW_VALUES/@Status)[1]', 'NVARCHAR(100)') <> CONVERT(XML, a.Data).value('(/Vendor/NEW_VALUES/OLD_VALUES/@Status)[1]', 'NVARCHAR(100)')
					--					THEN 1 
					--					ELSE 0 END))>0
										
					--					))
					--					OR a.LogData.value('(/VendorAttachment/NEW_VALUES/@VendorId)[1]', 'int') = @VendorId
					--		OR a.LogData.value('(/VendorAttachment/NEW_VALUES/OLD_VALUES/@VendorId)[1]', 'int') = @VendorId
				 --	 )
					-- )
