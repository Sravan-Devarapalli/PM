CREATE PROC [dbo].[spFinancialsRetrospectiveBudget]
(
	@ProjectIDs   IdsType READONLY,
	@StartDate   DATETIME,
	@EndDate     DATETIME
...
BEGIN
	...
	JOIN @ProjectIDs p on p.Id =PBPE.ProjectId
	...
	JOIN @ProjectIDs p on p.Id =PBH.ProjectId
	WHERE PBH.IsActive = 1 AND PBH.MilestoneId IS NULL
	...
	FROM #ProjectBudgetDiscount AS p 
		...
		INNER JOIN #CTE AS d ON d.date = cal.Date and m.MilestoneId = d.MileStoneId
		LEFT JOIN (select distinct milestoneid from dbo.ProjectBudgetFFMilestoneMonthlyRevenue PBF
					JOIN @ProjectIDs p on p.Id =PBF.ProjectId ) FMR on m.MilestoneId=FMR.MilestoneId
