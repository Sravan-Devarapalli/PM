...
AS
	SELECT cal.Date,
		   ...
		   --(CASE WHEN ISNULL(pcal.TimeTypeId, 0) = HTT.HolidayTimeTypeId  THEN 1 ELSE 0 END)   AS 'IsFloatingHoliday',
  		   (CASE WHEN pcal.TimeTypeId = HTT.HolidayTimeTypeId  THEN 1 ELSE 0 END)   AS 'IsFloatingHoliday', -- substitue for a holiday , worked on holiday and took other day as holiday
		   (CASE WHEN pcal.TimeTypeId = FHT.FHTimeTypeId  THEN 1 ELSE 0 END)   AS 'IsFloatingVacation',
		   pcal.ActualHours,
		   pcal.TimeTypeId,
		   pcal.Description,
		   pcal.ApprovedBy,
	  ...
	  FROM dbo.Calendar AS cal
		   ...
		   INNER JOIN dbo.GetFloatingHolidayTimeTypeIdTable() FHT ON 1=1
