...
-- Updated by:	
-- Update date:	
-- Description:	Logs the changes in the Skills.PersonProfile table.
-- =============================================
...
BEGIN
	...
	(
		SELECT i.Id AS ProfileId,
				...
				CASE WHEN i.IsDefault = 1 THEN 'YES' ELSE 'NO' END AS IsDefault
		  FROM inserted AS i
	...
	(
		SELECT d.Id AS ProfileId,
				...
				d.ProfileUrl,
				M.LastName + ',' + M.FirstName AS [ModifiedBy],
				CASE WHEN d.IsDefault = 1 THEN 'YES' ELSE 'NO' END AS IsDefault
		  FROM deleted AS d
