...
function setAlternateRows(tableObj){var rows=tableObj.getElementsByTagName("tr");
var i=0;
for(var j=0;
j<rows.length;
j++){if(rows[j].parentNode.parentNode.id==tableObj.id&&rows[j].style.display!="none"&&rows[j].parentNode.tagName.toUpperCase()=="TBODY"){if(i==0){rows[j].style.backgroundColor="white";
i=1
}else{rows[j].style.backgroundColor="#F5FAFF";
i=0
...
}else{if(config.textExtraction=="simple"){if(config.supportsTextContent){text=node.textContent
...
}function appendToTable(table,cache){if(cache.normalized.length>0){if(table.config.debug){var appendTime=new Date()
...
o.count=s[1];
o.count++
}}function multisort(table,sortList,cache){if(table.config.parsers!=undefined){if(table.config.debug){var sortTime=new Date()
